...
server {
    ...
    server_name     $hostname;
    ssl_certificate         /etc/nginx/ssl/web2py.pem;
...
then
  openssl req -new -x509 -sha256 -days 2000 -nodes -out web2py.pem -keyout web2py.key -newkey rsa:2048 -subj "/C=BR/ST=DF/L=Brasilia/O=Company/CN=www.company.com.br"
